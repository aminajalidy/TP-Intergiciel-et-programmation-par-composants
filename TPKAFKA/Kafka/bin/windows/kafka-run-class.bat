...
rem Add kafka specific gc options if they are not already set.
if not defined KAFKA_GC_LOG_OPTS (
    set KAFKA_GC_LOG_OPTS=-Xlog:gc*:file=%LOG_DIR%/kafkaServer-gc.log:time,tags:filecount=10,filesize=100M
)

rem Add kafka specific opts if they are not already set.
if not defined KAFKA_HEAP_OPTS (
    set KAFKA_HEAP_OPTS=-Xmx1024M -Xms512M
)
